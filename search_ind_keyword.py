...
publication_df = pd.DataFrame(columns=["Scholar", "Filtered paper title"])
individual_df = pd.read_csv("./individual.csv", keep_default_na=False)
for index, row in individual_df.iterrows():
    google_scholar_url = row["Google scholar URL"]
    print(google_scholar_url)
    if len(google_scholar_url) <= 0:
        continue
    ...
    for publication in publication_lst:
        for filter_keyword in filter_keywords:
            if filter_keyword in publication:
                ...
                break
    new_df = pd.DataFrame({"Scholar": [row["Name"]] * len(filtered_publication), "Filtered paper title": filtered_publication})
    publication_df = pd.concat([publication_df, new_df], ignore_index=True)
    # element_test = WebDriverWait(driver, 10).until(
    #         EC.element_to_be_clickable((By.XPATH, "//input[@id='inforet']"))
    #     )
publication_df.to_csv("publications.csv")
driver.quit()
